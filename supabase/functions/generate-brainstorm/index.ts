...
serve(async (req) => {
  ...
  try {
    const { destinations, styles, budget, travelMonth, numDays, arrivalCity, departureCity, notes, existingPlans, baseLocation } = await req.json();
    ...
    const userMessage = isOpenToIdeas
      ? `The traveler is OPEN TO IDEAS — they haven't decided on a destination yet.${baseLocation ? ` They are based in ${baseLocation} — prioritize destinations with easy/direct flight connections from there.` : ""} Suggest 4 completely different destinations around the world that would be ideal for their preferences.` +
