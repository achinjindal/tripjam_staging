...
serve(async (req) => {
  ...
  try {
    const { destinations: rawDest, styles, budget, travelMonth, numDays, arrivalCity, departureCity, notes, existingPlans, baseLocation } = await req.json();
    ...
    const userMessage = isOpenToIdeas
      ? `The traveler needs HELP DECIDING on a destination — they haven't chosen one yet.${baseLocation ? ` They are based in ${baseLocation}.` : ""} Suggest 4 completely different destinations around the world that would be ideal for their preferences.` +
