...
export function DestinationHero({ dest, isLoading, data, children }) {
  ...
  return (
    <div style={{borderRadius:RADIUS.lg,overflow:"hidden",border:`1px solid ${T.ocean}15`,marginBottom:4}}>
      {/* Hero photo */}
      ...
      <div style={{background:`linear-gradient(135deg, ${T.ocean}08, ${T.dusk}06)`,padding:"16px 18px"}}>
        {isLoading ? (
          <>
            {!photoUrl && <div style={{width:120,height:18,borderRadius:RADIUS.sm,background:T.sand,animation:"shimmer 1.5s ease-in-out infinite",marginBottom:10}}/>}
...
export function FoodSpotlightCard({ item, city }) {
  ...
  return (
    <div style={{ flexShrink: 0, width: 140, borderRadius: RADIUS.lg, overflow: "hidden", border: `1px solid ${T.warningBorder}`, background: T.warningLight }}>
      ...
      <div style={{ padding: "8px 10px 6px" }}>
        ...
        {item.note && <div style={{ fontSize: 10, color: T.warning, fontFamily: "Georgia,serif", lineHeight: 1.3 }}>{item.note}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: T.chalk, overflow: "hidden" }}>
      ...
      <div style={{ height: 180, background: T.sand, overflow: "hidden", position: "relative" }}>
        ...
        {/* City name badge */}
        <div style={{position:"absolute",top:12,left:12,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",color:"white",fontFamily:"'DM Serif Display',serif",fontSize:18,padding:"4px 14px",borderRadius:RADIUS.md}}>
          {city}
        </div>
        {/* Weather badge */}
        {dd?.weather && (
          <div style={{position:"absolute",bottom:8,right:8,background:"rgba(255,255,255,0.65)",backdropFilter:"blur(12px)",fontSize:11,padding:"6px 10px",borderRadius:RADIUS.md,color:T.ink,fontFamily:"Georgia,serif",fontWeight:600,maxWidth:200,lineHeight:1.4}}>
      ...
      {dd?.didYouKnow && (
        <div style={{margin:"0 18px 14px",padding:"12px 16px",borderLeft:`3px solid ${T.ocean}`,background:`linear-gradient(135deg, ${T.ocean}06, ${T.dusk}04)`,borderRadius:`0 ${RADIUS.lg}px ${RADIUS.lg}px 0`}}>
...
export function MagazineHighlightCard({ item, city, inItinerary = false, masonry = false, tall = false, onAskTrippy = null }) {
  ...
  return (
    <div style={{
      ...
      {inItinerary && (
        <div style={{position:"absolute",top:8,right:8,zIndex:2,background:T.ocean,borderRadius:"50%",width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"white",boxShadow:SHADOW.sm}}>✓</div>
      )}
      ...
      <div style={{ padding: "8px 10px 6px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 4 }}>
          ...
          <div style={{ display: "flex", gap: 3, flexShrink: 0 }}>
            {onAskTrippy && (
              <button onClick={() => onAskTrippy(item.title)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 22, height: 22, borderRadius: RADIUS.sm, border: `1px solid ${T.sand}`, background: T.chalk, cursor: "pointer", fontSize: 11, padding: 0 }}>💬</button>
            )}
            <a href={`https://www.google.com/maps/search/?api=1&query=${mapsQuery}`} target="_blank" rel="noopener noreferrer"
              style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 22, height: 22, borderRadius: RADIUS.sm, border: `1px solid ${T.sand}`, background: T.chalk, textDecoration: "none" }}>
        ...
        {inItinerary && <div style={{display:"inline-block",marginTop:5,fontSize:9,background:T.successLight,color:T.success,padding:"1px 7px",borderRadius:RADIUS.md,fontFamily:"Georgia,serif",fontWeight:600}}>In your itinerary</div>}
      </div>
    </div>
  );
}
...
export function HotelSuggestionCard({ suggestion, onSelect, onKnowMore }) {
  ...
  const priceColor = priceLen >= 4 ? "#7C3AED" : priceLen === 3 ? T.warning : T.moss;
  const priceBg   = priceLen >= 4 ? "#EDE9FE"  : priceLen === 3 ? T.warningLight : T.successLight;
  return (
    <div style={{ flexShrink:0, width:186, borderRadius:RADIUS.lg, overflow:"hidden", border:`1px solid ${T.sand}`, background:T.chalk, boxShadow:SHADOW.sm }}>
      {(!loaded || photoUrl) && (
        <div style={{ height:90, background:T.sand, overflow:"hidden", position:"relative" }}>
          {!loaded && <div style={{position:"absolute",inset:0,background:T.sand,animation:"shimmer 1.5s ease-in-out infinite"}}/>}
          ...
          {suggestion.price && (
            <div style={{position:"absolute",bottom:6,right:6,background:priceBg,color:priceColor,fontSize:10,fontFamily:"Georgia,serif",fontWeight:600,padding:"2px 7px",borderRadius:RADIUS.sm,letterSpacing:0.3}}>
              {suggestion.price}
            </div>
          )}
        </div>
      ...
      {loaded && !photoUrl && suggestion.price && (
        <div style={{padding:"6px 10px 0",textAlign:"right"}}>
          <span style={{background:priceBg,color:priceColor,fontSize:10,fontFamily:"Georgia,serif",fontWeight:600,padding:"2px 7px",borderRadius:RADIUS.sm,letterSpacing:0.3}}>{suggestion.price}</span>
        </div>
      )}
      ...
      <div style={{padding:"6px 8px 8px",display:"flex",gap:5}}>
        <button onClick={onSelect} style={{flex:1,padding:"5px 0",borderRadius:RADIUS.md,border:`1px solid ${T.ocean}`,background:T.ocean,fontFamily:"Georgia,serif",fontSize:10,color:"#fff",cursor:"pointer",transition:`background ${MOTION.fast}`}}>Use this</button>
        <button onClick={onKnowMore} style={{flex:1,padding:"5px 0",borderRadius:RADIUS.md,border:`1px solid ${T.sand}`,background:"transparent",fontFamily:"Georgia,serif",fontSize:10,color:T.mist,cursor:"pointer",transition:`background ${MOTION.fast}`}}>Know more</button>
